const flashcards = [
  { q: "What is the purpose of a literature review?", a: "To learn from existing studies and avoid repeating past work." },
  { q: "Name two reliable sources for research.", a: "Google Scholar, school library databases." },
  { q: "What’s a helpful reading strategy?", a: "Skim the abstract, intro, and conclusion first." }
];
...
function getProgress() {
  const data = JSON.parse(localStorage.getItem('week3_progress') || '{}');
...
function setProgress(section) {
  ...
  if (!data[section]) {
    data[section] = true;
    localStorage.setItem('week3_progress', JSON.stringify(data));
  }
}
...
function showCelebration() {
  ...
  msg.innerHTML = `
    <div id="celebrate-popup" style="position:fixed;top:120px;left:50%;transform:translateX(-50%) scale(0.8);z-index:10000;background:rgba(30,32,60,0.97);color:#00fff1;padding:2rem 2.5rem;border-radius:18px;box-shadow:0 0 32px #00fff1,0 0 64px #a259ff;font-size:1.3rem;font-weight:700;text-align:center;opacity:0;animation:popInPulse 0.7s cubic-bezier(.23,1,.32,1) forwards;">
      ...
      🎉 Congratulations! You completed Week 3.<br><br>
...
function clearProgress() {
  localStorage.removeItem('week3_progress');
  lastPercent = 0;
  updateProgressBar();
