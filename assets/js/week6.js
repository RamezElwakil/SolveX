const flashcards = [
  { q: "What is documentation in research?", a: "A record of everything you did, used, and observed during your project." },
  { q: "What are 3 things you should document?", a: "Procedure, materials, and data." },
  { q: "Why is documenting challenges helpful?", a: "It shows how you adapted and what others can learn from your experience." }
];
...
function getProgress() {
  const data = JSON.parse(localStorage.getItem('week6_progress') || '{}');
...
function setProgress(section) {
  ...
  if (!data[section]) {
    data[section] = true;
    localStorage.setItem('week6_progress', JSON.stringify(data));
  }
}
...
function showCelebration() {
  ...
  msg.innerHTML = `
    <div id="celebrate-popup" style="position:fixed;top:120px;left:50%;transform:translateX(-50%) scale(0.8);z-index:10000;background:rgba(30,32,60,0.97);color:#00fff1;padding:2rem 2.5rem;border-radius:18px;box-shadow:0 0 32px #00fff1,0 0 64px #a259ff;font-size:1.3rem;font-weight:700;text-align:center;opacity:0;animation:popInPulse 0.7s cubic-bezier(.23,1,.32,1) forwards;">
      ...
      🎉 Congratulations! You completed Week 6.<br><br>
...
function clearProgress() {
  localStorage.removeItem('week6_progress');
  lastPercent = 0;
  updateProgressBar();
