const flashcards = [
  { q: "What is an independent variable?", a: "The thing you change in an experiment." },
  { q: "What is a dependent variable?", a: "The thing you measure." },
  { q: "Why are controlled variables important?", a: "To keep the test fair and results valid." }
];
...
function getProgress() {
  const data = JSON.parse(localStorage.getItem('week4_progress') || '{}');
...
function setProgress(section) {
  ...
  if (!data[section]) {
    data[section] = true;
    localStorage.setItem('week4_progress', JSON.stringify(data));
  }
}
...
function showCelebration() {
  ...
  msg.innerHTML = `
    <div id="celebrate-popup" style="position:fixed;top:120px;left:50%;transform:translateX(-50%) scale(0.8);z-index:10000;background:rgba(30,32,60,0.97);color:#00fff1;padding:2rem 2.5rem;border-radius:18px;box-shadow:0 0 32px #00fff1,0 0 64px #a259ff;font-size:1.3rem;font-weight:700;text-align:center;opacity:0;animation:popInPulse 0.7s cubic-bezier(.23,1,.32,1) forwards;">
      ...
      🎉 Congratulations! You completed Week 4.<br><br>
...
function clearProgress() {
  localStorage.removeItem('week4_progress');
  lastPercent = 0;
  updateProgressBar();
