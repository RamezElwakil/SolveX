const flashcards = [
  { q: "What are key parts of a presentation?", a: "Question, method, data, conclusion." },
  { q: "Why is evaluation important?", a: "To understand results and improve your project." },
  { q: "Name one way to present your project.", a: "Poster, slideshow, video." }
];
...
function handleQuizSubmit(e) {
  ...
  document.getElementById('quiz-feedback').innerHTML =
    `<div style=\"margin-bottom:0.7em;\">You got <b>${correct}/2</b> correct.</div>${feedback}`;
}

function getProgress() {
  const data = JSON.parse(localStorage.getItem('week7_progress') || '{}');
...
function setProgress(section) {
  ...
  if (!data[section]) {
    data[section] = true;
    localStorage.setItem('week7_progress', JSON.stringify(data));
  }
}
...
function showCelebration() {
  ...
  msg.innerHTML = `
    <div id="celebrate-popup" style="position:fixed;top:120px;left:50%;transform:translateX(-50%) scale(0.8);z-index:10000;background:rgba(30,32,60,0.97);color:#00fff1;padding:2rem 2.5rem;border-radius:18px;box-shadow:0 0 32px #00fff1,0 0 64px #a259ff;font-size:1.3rem;font-weight:700;text-align:center;opacity:0;animation:popInPulse 0.7s cubic-bezier(.23,1,.32,1) forwards;">
      ...
      🎉 Congratulations! You completed Week 7.<br><br>
...
function clearProgress() {
  localStorage.removeItem('week7_progress');
  lastPercent = 0;
  updateProgressBar();
